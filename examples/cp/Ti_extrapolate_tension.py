...
def make_Ti_polycrystal(N, nthreads):
    ...
    # smodel = parse.parse_xml("ti_tpsingle_units.xml", "forest")

    orientations = rotations.random_orientations(N)
    # orientations = np.array([rotations.CrystalOrientation(0, 0, 0, angle_type = "degrees", convention = "kocks")]* N)
    model = polycrystal.TaylorModel(smodel, orientations, nthreads=nthreads)
    smodel.save("ti_tpsingle_update.xml", "forest")
    return smodel
