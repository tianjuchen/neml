...
from Ti_maker import extrapolate
...
def Ti_singlecrystal(
    ...
    if return_hardening:
        return single_model, strength, lattice
    else:
        return single_model, lattice
...
if __name__ == "__main__":
    ...
    smodel, lattice = Ti_singlecrystal(
        verbose=True, PTR=True, return_hardening=False, update_rotation=True
    )
    ...
    plt.savefig("initialpf-%i-C.png" % int(T - 273.15), dpi=300)
    plt.show()
    plt.close()
    ...
    # plt.savefig("ss-%i-C-1.png" % int(T - 273.15), dpi=300)
    plt.show()
    plt.close()
    ...
    plt.savefig("deformpf-%i-C-1.png" % int(T - 273.15), dpi=300)
    # plt.show()
    plt.close()
