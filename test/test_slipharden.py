...
class TestConstantHardening(unittest.TestCase, CommonSlipHardening):
    ...
    def test_hist_to_tau(self):
        for g in range(self.L.ngroup):
            for i in range(self.L.nslip(g)):
                model = self.model.hist_to_tau(g, i, self.H, self.L, self.T, self.fixed)
                should = self.static
                self.assertAlmostEqual(model, should)

    def test_definition(self):
        hrate = self.model.hist(
            self.S, self.Q, self.H, self.L, self.T, self.sliprule, self.fixed
        )
        exact = np.array([])
...
class TestGeneralLinearHardeningNoAbs(unittest.TestCase, CommonSlipHardening):
    def setUp(self):
        ...
        self.M = matrix.SquareMatrix(
            self.nslip, type="block", data=[0.1, 0.2, 0.3, 0.4], blocks=[6, 6]
        )

        self.s0 = [self.static] * self.nslip

        self.model = slipharden.GeneralLinearHardening(self.M, self.s0, absval=False)
    ...
    def test_definition(self):
        ...
        srates = [
            self.sliprule.slip(g, i, self.S, self.Q, self.H, self.L, self.T, self.fixed)
            for g in range(self.L.ngroup)
            for i in range(self.L.nslip(g))
        ]
...
class TestGeneralLinearHardeningAbs(unittest.TestCase, CommonSlipHardening):
    def setUp(self):
        ...
        self.model = slipharden.GeneralLinearHardening(self.M, self.s0, absval=True)
...
class TestLANLTiModel(unittest.TestCase, CommonSlipHardening):
    def setUp(self):
        ...
        self.S = (
            tensors.Symmetric(
                np.array(
                    [[100.0, -25.0, 10.0], [-25.0, -17.0, 15.0], [10.0, 15.0, 35.0]]
                )
            )
            * 2
        )
        ...
        self.rhos = np.array([2, 3, 2, 2, 3, 2, 2, 1, 4, 5, 1, 2.0]) * 2.0e-2
        ...
        self.model = slipharden.LANLTiModel(
            self.tau0, self.G, self.mu, self.k1, self.k2, X_s=self.X_s
        )
    ...
    def test_hist_to_tau(self):
        ...
        check[:12] = (
            self.X_s * burger[:12] * self.mu[:12] * np.array(self.H)[:12]
            + self.tau0[:12]
        )
        check[12:] = (
            self.G_np.dot(np.array(self.H)[:12] ** 2.0 * burger[:12])
            * self.mu[12:]
            * burger[12:]
            + self.tau0[12:]
        )
...
class CommonSlipSingleHardening:
    def test_d_hist_map(self):
        nd = diff_history_scalar(
            lambda h: self.model.hist_map(h, self.T, self.fixed), self.H
        )
    ...
    def test_hist_to_tau(self):
        for g in range(self.L.ngroup):
            for i in range(self.L.nslip(g)):
                self.assertTrue(
                    np.isclose(
                        self.strength + self.static + self.nye_part,
                        self.model.hist_to_tau(
                            g, i, self.H, self.L, self.T, self.fixed
                        ),
                    )
                )
...
class CommonSlipSingleStrengthHardening:
    def test_hist_map(self):
        self.assertTrue(
            np.isclose(
                self.model.hist_map(self.H, self.T, self.fixed),
                self.strength + self.static + self.nye_part,
            )
        )
    ...
    def test_hist(self):
        ...
        self.assertTrue(
            np.isclose(
                h.get_scalar(self.vname),
                self.model.hist_rate(
                    self.S, self.Q, self.H, self.L, self.T, self.sliprule, self.fixed
                ),
            )
        )
    ...
    def test_d_hist_rate_d_stress(self):
        dfn = lambda s: self.model.hist_rate(
            s, self.Q, self.H, self.L, self.T, self.sliprule, self.fixed
        )
        nd = diff_scalar_symmetric(dfn, self.S)
        d = self.model.d_hist_rate_d_stress(
            self.S, self.Q, self.H, self.L, self.T, self.sliprule, self.fixed
        )
        self.assertEqual(d, nd)

    def test_d_hist_rate_d_hist(self):
        dfn = lambda h: self.model.hist_rate(
            self.S, self.Q, h, self.L, self.T, self.sliprule, self.fixed
        )
        nd = diff_history_scalar(dfn, self.H)

        d = self.model.d_hist_rate_d_hist(
            self.S, self.Q, self.H, self.L, self.T, self.sliprule, self.fixed
        )
    ...
    def test_nye_parts_match(self):
        self.assertTrue(
            np.isclose(
                self.model.nye_part(self.nye, self.T),
    ...
    def test_no_nye_works(self):
        self.assertTrue(
            np.isclose(self.model.nye_contribution(history.History(), self.T), 0.0)
        )
    ...
    def test_hist_rate(self):
        ...
        self.assertTrue(
            np.isclose(
                ...
                self.model.hist_factor(
                    self.H.get_scalar(self.vname), self.L, self.T, self.fixed
                )
                * ss,
            )
        )
...
class TestVoceHardening(
    unittest.TestCase,
    CommonPlasticSlipHardening,
    CommonSlipSingleStrengthHardening,
    CommonSlipSingleHardening,
    CommonSlipHardening,
):
    def setUp(self):
        self.vname = "strength"
    ...
    def test_nye_factor(self):
        self.assertTrue(np.isclose(self.model.nye_part(self.nye, self.T), 0.0))

    def test_use_nye(self):
        self.assertFalse(self.model.use_nye)


class TestNyeVoceHardening(
    unittest.TestCase,
    CommonPlasticSlipHardening,
    CommonSlipSingleStrengthHardening,
    CommonSlipSingleHardening,
    ...
    def setUp(self):
        ...
        self.model = slipharden.VoceSlipHardening(
            self.tau_sat, self.b, self.tau0, k=self.k
        )
...
class TestVoceHardeningMoveVar(
    unittest.TestCase,
    CommonPlasticSlipHardening,
    CommonSlipSingleStrengthHardening,
    CommonSlipSingleHardening,
    ...
    def test_nye_factor(self):
        self.assertTrue(
            np.isclose(self.model.nye_part(self.nye, self.T), self.nye_part)
        )
...
class TestMultiVoceHardening(
    unittest.TestCase, CommonSlipSingleHardening, CommonSlipHardening
):
    def setUp(self):
        self.L = crystallography.CubicLattice(1.0)
        self.L.add_slip_system([1, 1, 0], [1, 1, 1])
        ...
        self.model1 = slipharden.VoceSlipHardening(
            self.tau_sat_1, self.b_1, self.tau0_1
        )
        self.model2 = slipharden.VoceSlipHardening(
    ...
    def test_hist_def(self):
        hrate_1 = (
            self.b_1
            * (self.tau_sat_1 - self.strength_0)
            * self.sliprule.sum_slip(self.S, self.Q, self.H, self.L, self.T, self.fixed)
        )
        hrate_2 = (
            self.b_2
            * (self.tau_sat_2 - self.strength_1)
            * self.sliprule.sum_slip(self.S, self.Q, self.H, self.L, self.T, self.fixed)
        )
